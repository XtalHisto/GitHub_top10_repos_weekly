...
diffs = snapshoter.get_top_growth_repos_safe(current_date=curr_date, previous_date=pre_date)
for diff in diffs['results']:
    diff['summary'] = analysier.get_response(diff, diffs['time_delta'])
for people in cfg.recipient:
    html_content = builder.build_html(diffs['results'], people['name'] ) 
